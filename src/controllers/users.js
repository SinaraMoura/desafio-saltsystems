...
const usersRegistration = async (req, res) => {
    ...
    try {
        const queryVerifyEmail = await knex('users').where('email', email);
        if (queryVerifyEmail.length > 0) {
            return res.status(404).json({ message: "E-mail informado já existente." });
        }
        const
            encryptedPassword = await bcrypt.hash(password, 10);
        const queryRegistration = await knex('users').insert({ name, email, password: encryptedPassword }).returning('*');
...
module.exports = { usersRegistration, login, verifyEmail, verifyUser }